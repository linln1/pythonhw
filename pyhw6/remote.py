...
async def send_data_to_server(cl_reader, rm_writer, logHint= None, user=None):
    try:
        uBucket = uBucketdict[user]
        ...
        while True:
            ...
            await uBucket.lock.acquire()
            c.execute("UPDATE USER SET BANDWITH=? WHERE USERNAME=?", (tokenNum, user,))
            await uBucket.lock.release()
            ...
            async with db.execute('SELECT * FROM USER WHERE USERNAME=? and PASSWORD=?', (username, password, ) ) as cur:
                ...
                if rm_writer:
                    ...
                    await asyncio.wait({
                        ...
                        asyncio.create_task(send_data_to_server(reader, rm_writer, user=username)),
